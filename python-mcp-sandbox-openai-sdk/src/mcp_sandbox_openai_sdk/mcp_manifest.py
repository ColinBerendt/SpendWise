...
class DevMCPManifest(MCPManifest):
    """
    Special version of the MCP Manifest that instructs the sandbox to use "pre installed" packages.
    Thus, no installation steps are executed inside the sandbox image.

    This manifest requires you to mount the executable code (with all dependencies installed)
    into the sandbox container and provide the command to execute the code.
