#!/usr/bin/env python3
"""
Import MockBank Data - Directly imports MockBank demo data into SpendWise

This script:
1. Contains all MockBank transactions (copy of their seed data)
2. Auto-categorizes based on merchant names
3. Inserts directly into SpendWise database
4. NO MockBank API needed!

Usage:
    cd SpendWise
    uv run python testing/import_mockbank_data.py
"""
...
# Category IDs (must match existing categories in DB)
# 1 = Food & Dining
# 2 = Transportation
# 3 = Shopping
# 4 = Entertainment
...
def get_category(description: str) -> int:
    """Map merchant/description to category ID."""
    desc = description.upper()
    
    # Income (8)
    if any(x in desc for x in ["SALARY", "BONUS", "CARRYOVER", "TRANSFER" if "CAYMAN" not in desc else ""]):
        if "CAYMAN" not in desc:  # Suspicious transfer not income
            return 8
    
    # Food & Dining (1)
    if any(x in desc for x in [
        "MIGROS", "COOP", "DENNER", "MCDONALD", "STARBUCKS", 
        "RESTAURANT", "KEBAB", "PIZZA", "SUSHI", "FONDUE",
        "CHINESE", "THAI", "ITALIANO", "INDIAN", "FRENCH",
        "ASIA", "DINNER", "KRONENHALLE", "BEACH CLUB"
    ]):
        return 1
    
    # Transportation (2)
    if any(x in desc for x in [
        "SBB", "ZVV", "UBER", "PARKING", "SHELL", "BP", "FLIGHT", "SWISS"
    ]):
        return 2
    
    # Shopping (3)
    if any(x in desc for x in [
        "AMAZON", "ZALANDO", "H&M", "IKEA", "DIGITEC", "MEDIAMARKT",
        "MANOR", "FLOWERS", "COSTUME", "LUXURY WATCHES", "BOOKSTORE",
        "ORELL"
    ]):
        return 3
    ...
    # Entertainment (4)
    if any(x in desc for x in [
        "NETFLIX", "SPOTIFY", "STEAM", "CINEMA", "CONCERT", 
        "HALLENSTADION", "VOLKSHAUS", "OPENAIR", "NIGHT CLUB"
    ]):
        return 4
    
    # Bills & Utilities (5)
    if any(x in desc for x in [
        "RENT", "SWISSCOM", "EWZ", "ELECTRICITY", "CSS INSURANCE", 
        "SERAFE", "HOTEL"
    ]):
        return 5
    ...
    # Health (6)
    if any(x in desc for x in [
        "PHARMACY", "AMAVITA", "DENTIST", "DR.", "FITNESS", "MUELLER"
    ]):
        return 6
    
    # Education (7)
    if any(x in desc for x in [
        "ETH", "UDEMY", "COURSERA", "BOOKS", "TUITION", "SEMESTER"
    ]):
        return 7
    
    # Suspicious / Other (10)
    if any(x in desc for x in [
        "CASINO", "CRYPTO", "WEAPONS", "ANONYMOUS", "CAYMAN",
...
# All MockBank transactions (2025) - with hardcoded references matching MockBank
...
TRANSACTIONS_2025 = [
    ...
    ("2025-11-15", "BP ZURICH", -1500.00, "TXN-00417"),
    ("2025-09-08", "SHELL BERN", -2200.00, "TXN-00418"),
    ("2025-07-22", "WEAPONS STORE SOMALIA", -3500.00, "TXN-00419"),
    ("2025-05-18", "ANONYMOUS TRANSFER", -5000.00, "TXN-00420"),
    ("2025-03-12", "ANONYMOUS", 8000.00, "TXN-00421"),
    ("2025-10-25", "CASINO ONLINE", -1200.00, "TXN-00422"),
    ("2025-08-14", "CRYPTO EXCHANGE", -2500.00, "TXN-00423"),
    ("2025-06-28", "CASH WITHDRAWAL ATM", -9000.00, "TXN-00424"),
    ("2025-04-19", "UNKNOWN MERCHANT XYZ", -750.00, "TXN-00425"),
    ("2025-02-22", "TRANSFER CAYMAN ISLANDS", -12000.00, "TXN-00426"),
    ("2025-12-05", "NIGHT CLUB IBIZA", -890.00, "TXN-00427"),
    ("2025-01-28", "LUXURY WATCHES GENEVA", -4500.00, "TXN-00428"),
]

# Weekly budgets
BUDGETS = [
    (1, 200.00),   # Food: 200 CHF/week
    (2, 150.00),   # Transportation: 150 CHF/week
    (3, 100.00),   # Shopping: 100 CHF/week
...
def extract_merchant(description: str) -> str:
    ...
    if len(parts) >= 2:
        # Check if last part is a city
        cities = ["ZURICH", "BERN", "BASEL", "GENEVA", "LAUSANNE", "WINTERTHUR", 
                  "SPREITENBACH", "DIETLIKON", "LUZERN", "BARCELONA", "IBIZA", 
                  "SOMALIA", "ST.", "GALLEN"]
        if parts[-1].upper() in cities:
            return " ".join(parts[:-1])
        if len(parts) >= 3 and parts[-2].upper() == "ST.":
            return " ".join(parts[:-2])
    return description
...
def main():
    print("\n" + "=" * 60)
    print("  SpendWise - Import MockBank Data")
    print("=" * 60)
    
    if not DB_PATH.exists():
        print(f"\nERROR: Database not found at {DB_PATH}")
        print("Run: uv run python testing/seed_data.py first!")
        return
    ...
    print("\n[OK] Cleared existing transactions")
    ...
    # Summary
    print(f"\n[OK] Imported {len(TRANSACTIONS_2025)} transactions")
    ...
    for cat_id, count in sorted(category_counts.items()):
        ...
        print(f"  {cat_id}. {cat_name}: {count} transactions")
    ...
    print(f"\n--- Summary ---")
    print(f"  Total Income:   CHF {income:,.2f}")
    print(f"  Total Expenses: CHF {abs(expenses):,.2f}")
    print(f"  Net:            CHF {income + expenses:,.2f}")
    
    conn.close()
    
    print("\n" + "=" * 60)
    print("  Done! MockBank data imported successfully.")
    print("=" * 60 + "\n")
